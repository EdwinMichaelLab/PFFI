...
%  
% Description: In the absence of age-stratified infection data, age
% infection profiles are constructed based on characteristic curve shapes
% and the overall community prevalence. This derived theoretical data is
...
SIR_samples = round(SIR_samples/3);

for icurve = 1:3 % loop over three types of age curves
    ...
    MfData = getMfAgeProfile_fromLFCurves(MfData0(:,2),icurve,OverallMfPrev,ageMthMax/12,demog,MidAge);
    ...
    parfor i=1:length(mfPrevArray(1,:))
        ...
        LikArray2(i)=LikArray2(i)+...
            likelihood_using_ranks_by_passFail_Mf_Only(mfPrevArray(:,i),MfBounds);
    end
