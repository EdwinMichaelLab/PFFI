...
for iSites = 1:length(Sites)
    ...
    if isnan(ABR)          
        ABR0 = ABRmin + (ABRmax-ABRmin)*lhsdesign(NParamVecs,1,'criterion','correlation');
    else
        ABR0 = ones(NParamVecs,1).*ABR;
    end
    
    RunSelectParameterVectors(Sites{iSites},NParamVecs,MfData,ABR0,...
